...
if NumAdmin == 0 then 
goker_sendMsg(msg.chat_id_,msg.id_,"♦️┇لا يـوجـد أدمـنـيـه لكي يتـم رفعهم \n")
else
goker_sendMsg(msg.chat_id_,msg.id_,"♦️┇تم رفع  { *"..NumAdmin.."* } مـن آلآدمـنيهہ‌‏ في آلبوت \n✓️")
end
end,100)
end
...
if MAXBOT:get(max..'group:add'..msg.chat_id_) then  return '🎗*┊*المجموعه بالتأكيد ✓️ تم تفعيلها' end
...
if not MAXBOT:get(max..'group:add'..msg.chat_id_) then return '🛠*┊* المجموعه بالتأكيد ✓️ تم تعطيلها' end  
...
if cmd == "silent" then
if UserID == our_id then   
goker_sendMsg(ChatID,MsgID,"🏌‍♂*┊*لا يمكنك كتم البوت\n🛠") 
elseif UserID == SUDO_ID then 
goker_sendMsg(ChatID,MsgID,"🏌‍♂*┊*لا يمكنك كتم المطور الاساسي\n🛠") 
elseif MAXBOT:sismember(max..':SUDO_BOT:',UserID) then 
goker_sendMsg(ChatID,MsgID,"🏌‍♂*┊*لا يمكنك كتم المطور\n🛠") 
elseif MAXBOT:sismember(max..':MONSHA_BOT:'..ChatID,UserID) then 
goker_sendMsg(ChatID,MsgID,"🏌‍♂*┊*لا يمكنك كتم المنشئ\n🛠") 
elseif MAXBOT:sismember(max..'owners:'..ChatID,UserID) then 
goker_sendMsg(ChatID,MsgID,"🏌‍♂*┊*لا يمكنك كتم المدير\n🛠") 
elseif MAXBOT:sismember(max..'admins:'..ChatID,UserID) then 
goker_sendMsg(ChatID,MsgID,"🏌‍♂*┊*لا يمكنك كتم الادمن\n🛠") 
end
...
if cmd == "iduser" then
goker_sendMsg(ChatID,MsgID,"🧟‍♂*¦* آضـغط على آلآيدي ليتم آلنسـخ\n\n "..UserName.." ~⪼ ( `"..UserID.."` )")
end
...
if MAXBOT:sismember(max..'admins:'..ChatID,UserID) then 
goker_sendMsg(ChatID,MsgID,"🏌‍♂*┊*لا يمكنك كتم المدراء او الادمنيه\n🛠") 
end
